...
@onready var Spawn=preload("res://Scene/Character.tscn")
@onready var EnnemySpawn=preload("res://Scene/Ennemy.tscn")
...
func connect_rooms(room1, room2, corridor_width=1):
	...
	if spa:
		var player=Spawn.instantiate()
		spa=false
		player.position=16*start
		add_child(player)
	...
	var RA = rng.randf_range(-10.0, 10.0)
	if RA<5:
		var ennemy=EnnemySpawn.instantiate()
		ennemy.position=16*end
		add_child(ennemy)
	var current = start
	...
	for x in range(WIDTH):
		for y in range(HEIGHT):
			...
			if grid[x][y]!=0:
				...
				tile_map.set_cell(0,Vector2i(x,y),0, Vector2i(4,2))
